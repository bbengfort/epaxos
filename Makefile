# Scripts to handle ePaxos build and installation
...
.PHONY: all install build epaxos deps test citest clean doc protobuf
...
install: build
	$(info $(GM) installing epaxos and making configuration …)
	@ cp $(BUILD)/epaxos /usr/local/bin/

# Build the various binaries and sources
build: protobuf epaxos

# Build the epaxos command and store in the build directory
epaxos:
	$(info $(GM) compiling epaxos executable …)
	@ $(GOBUILD) -o $(BUILD)/epaxos ./cmd/epaxos
...
doc:
	...
	@ open http://localhost:6060/pkg/github.com/bbengfort/epaxos/
	@ $(GODOC) --http=:6060
